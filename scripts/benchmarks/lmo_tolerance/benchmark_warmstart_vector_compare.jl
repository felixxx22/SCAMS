...
function _build_hand_vector(n_local::Int, strategy::AbstractString)
    if strategy == "with_vector"
        return ones(Float64, n_local)
    end
    return nothing
end
...
function _run_one_c_mode(input_file::AbstractString, log_file::AbstractString;
    ...
    warmstart_strategy="without_vector",
)
    io_start_ns = time_ns()
    ...
    v0 = B(A, d=1 / m)
    initvec = _build_hand_vector(size(C, 1), warmstart_strategy)
    reuse_leading = warmstart_strategy == "with_vector"
    ...
    solve_result = Solve(
        ...
        arnoldi_initial_vector=initvec,
        warmstart_reuse_leading_eigvec=reuse_leading,
    )
...
function run_warmstart_vector_compare(; 
    phase="subset",
    repeats=3,
    warmup=true,
    ...
    strategies = ["without_vector", "with_vector"]
    ...
    for ds in dataset_specs
        ...
        for g in ds.specs
            ...
            if warmup
                warmup_tag = string(ds.dataset, "_", _graph_stem(g.name), "_warmup_without_vector")
                if resume_if_possible && (warmup_tag in existing_tags)
                    println("[warmstart-compare] skip existing warmup tag=", warmup_tag)
                else
                    _run_one_c_mode(
                        g.path,
                        summary_csv,
            ...
            for strategy in strategies
                for rep in 1:repeats
                    ...
                    if resume_if_possible && (run_tag in existing_tags)
                        println("[warmstart-compare] skip existing run tag=", run_tag)
                        continue
                    end
                    ...
                    push!(run_rows, [
                        ...
                        result.bench.warmstart_custom_init_accepted,
                        result.bench.first_lmo_init_source,
                    ])
                    push!(existing_tags, run_tag)

                    println(@sprintf(
                        "[warmstart-compare] dataset=%s graph=%s strategy=%s rep=%d solve=%.3fs fw=%d lmo=%.3fs final_gap=%.3e",
                        ds.dataset,
                        g.name,
                        strategy,
                        rep,
    ...
    println("\nWarm-start vector comparison run complete.")
    println("Phase: ", phase)
