...
function _strategy_flags(strategy::AbstractString)
    if strategy == "random"
        return (initvec=nothing, reuse_leading=false, scaled_warmstart=false)
    elseif strategy == "warmstart_naive"
        return (initvec=nothing, reuse_leading=true, scaled_warmstart=false)
    elseif strategy == "warmstart_scaled"
        return (initvec=nothing, reuse_leading=false, scaled_warmstart=true)
    else
...
function _run_one_c_mode(input_file::AbstractString, log_file::AbstractString;
    ...
    v0 = B(A, d=1 / m)
    flags = _strategy_flags(warmstart_strategy)
    ...
    solve_result = Solve(
        ...
        arnoldi_initial_vector=flags.initvec,
        warmstart_reuse_leading_eigvec=flags.reuse_leading,
        scaled_warmstart=flags.scaled_warmstart,
    )
...
function _aggregate_by_strategy(run_rows::Vector{Vector})
    grouped = Dict{Tuple{String, String}, Vector{Vector}}()
    for row in run_rows
        key = (String(row[1]), String(row[8]))
    ...
    for ((dataset, strategy), rows) in sort(collect(grouped), by=x -> (x[1][1], x[1][2]))
        fw_values = [Float64(r[14]) for r in rows]
        lmo_values = [Float64(r[16]) for r in rows]
        solve_values = [Float64(r[13]) for r in rows]
        gap_values = [Float64(r[17]) for r in rows]
...
function _aggregate_by_graph_strategy(run_rows::Vector{Vector})
    grouped = Dict{Tuple{String, String, String}, Vector{Vector}}()
    for row in run_rows
        key = (String(row[1]), String(row[2]), String(row[8]))
        if !haskey(grouped, key)
            grouped[key] = Vector{Vector}()
        end
        push!(grouped[key], row)
    ...
    out_rows = Vector{Vector}()
    for ((dataset, graph_name, strategy), rows) in sort(collect(grouped), by=x -> (x[1][1], x[1][2], x[1][3]))
        fw_values = [Float64(r[14]) for r in rows]
        lmo_values = [Float64(r[16]) for r in rows]
        solve_values = [Float64(r[13]) for r in rows]
        gap_values = [Float64(r[17]) for r in rows]
...
function run_warmstart_strategy_compare(;
    phase="subset",
    repeats=3,
    warmup=true,
    ...
    outdir = _project_path("Result", "benchmarks_warmstart_strategy_compare", phase)
    mkpath(outdir)
    ...
    strategies = ["random", "warmstart_naive", "warmstart_scaled"]
    ...
    for ds in dataset_specs
        ...
        for g in ds.specs
            ...
            if warmup
                warmup_tag = string(ds.dataset, "_", _graph_stem(g.name), "_warmup_random")
                if resume_if_possible && (warmup_tag in existing_tags)
                    println("[warmstart-strategy-compare] skip existing warmup tag=", warmup_tag)
                else
                    _run_one_c_mode(
                        g.path,
                        summary_csv,
            ...
            for strategy in strategies
                for rep in 1:repeats
                    run_tag = string(ds.dataset, "_", _graph_stem(g.name), "_", strategy, "_run", rep)

                    if resume_if_possible && (run_tag in existing_tags)
                        println("[warmstart-strategy-compare] skip existing run tag=", run_tag)
                        continue
                    end
                    ...
                    push!(run_rows, [
                        ...
                        result.bench.warmstart_reuse_leading_eigvec,
                        result.bench.scaled_warmstart,
                        result.bench.first_lmo_init_source,
                    ])
                    push!(existing_tags, run_tag)

                    println(@sprintf(
                        "[warmstart-strategy-compare] dataset=%s graph=%s strategy=%s rep=%d solve=%.3fs fw=%d lmo=%.3fs final_gap=%.3e",
                        ds.dataset,
                        g.name,
                        strategy,
                        rep,
    ...
    _write_csv(
        ...
        [
            ...
            "strategy", "linesearch", "epsilon", "start_epsilon_d0", "tighten_arnoldi_tol",
            ...
            "warmstart_reuse_leading_eigvec", "scaled_warmstart", "first_lmo_init_source",
        ],
        run_rows,
    )

    by_strategy_rows = _aggregate_by_strategy(run_rows)
    _write_csv(
        joinpath(outdir, "by_strategy.csv"),
        [
            "dataset", "strategy", "runs",
    ...
    by_graph_strategy_rows = _aggregate_by_graph_strategy(run_rows)
    _write_csv(
        joinpath(outdir, "by_graph_strategy.csv"),
        [
            "dataset", "graph_name", "strategy", "runs",
            "mean_fw_iterations", "std_fw_iterations",
            "mean_total_lmo_time_sec", "std_total_lmo_time_sec",
            "mean_solve_time_sec", "std_solve_time_sec",
            "mean_final_gap", "std_final_gap", "min_final_gap", "max_final_gap",
        ],
        by_graph_strategy_rows,
    )

    expected_runs = (length(gset_specs) + length(big_specs)) * length(strategies) * repeats

    println("\nWarm-start strategy comparison run complete.")
    println("Phase: ", phase)
    println("Runs CSV: ", runs_csv)
    println("By-strategy CSV: ", joinpath(outdir, "by_strategy.csv"))
    println("By-graph/strategy CSV: ", joinpath(outdir, "by_graph_strategy.csv"))
...
if abspath(PROGRAM_FILE) == @__FILE__
    run_warmstart_strategy_compare(
        phase="subset",
        repeats=3,
        warmup=true,
        resume_if_possible=true,
